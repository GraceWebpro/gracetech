...
const Dashboard = () => {
  ...
  const [downloads, setDownloads] = useState([]); // for free + paid downloads
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const downloadsData = downloadsSnap.docs.map(d => ({ id: d.id, ...d.data() }));
  ...
  const activities = [
    ...orders.map(o => ({
      id: o.id,
      type: "purchase",
      label: `Purchased ${o.templateName}`,
      date: o.purchaseDate,
    })),
    ...downloads.map(d => ({
      id: d.id,
      type: "download",
      label: `Downloaded ${d.templateName}`,
      date: d.downloadDate,
    })),
  ]
    .filter(a => a.date)
  ...
  const totalSpent = orders.reduce((a, b) => a + (b.amountPaid || 0), 0);
  ...
  return (
    <div className="dashboard">
      ...
      >
        ...
        <div style={{ gap: '90px' }}
          className="flex items-center justify-between md:justify-start mt-10">
          <h2 className="text-xl font-bold text-white">GraceTech</h2>
          <button
...
//       ${active ? "bg-gradient-to-r from-purple-500 to-cyan-400 text-black" : "text-white/60 hover:bg-white/10"}
//       w-full
//     `}
//   >
